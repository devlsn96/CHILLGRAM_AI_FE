              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  ...
                  {!isLoading &&
                    !isError &&
                    Array.from({
                      length: Math.max(0, pageSize - filteredProducts.length),
                    }).map((_, i) => (
                      <tr
                        key={`empty-${i}`}
                        className="border-b border-gray-50 h-[73px]"
                      >
                        <td
                          colSpan="7"
                          className="py-4 px-4 text-center text-gray-300 text-sm"
                        >
                          {/* 데이터가 아예 없을 때 첫 번째 빈 행에 메시지 표시 */}
                          {filteredProducts.length === 0 && i === 4
                            ? "검색 결과가 없습니다"
                            : ""}
                        </td>
                      </tr>
